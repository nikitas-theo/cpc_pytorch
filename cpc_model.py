...
class CPC_encoder(Module):
    ...
    def __init__(
        ...
        self.conv_layer_3 = Conv1d(
            in_channels=conv_3_in_dim,
        ...
        self.conv_layer_4 = Conv1d(
            in_channels=conv_4_in_dim,
            out_channels=conv_4_out_dim,
        ...
        self.conv_layer_5 = Conv1d(
            in_channels=conv_5_in_dim,
            out_channels=conv_5_out_dim,
    ...
    def forward(self, X):
        ...
        X = X.unsqueeze(1)
        X = self.dropout(
            self.non_linearity_relu(self.batch_normalization_1(self.conv_layer_1(X)))
        )
        X = self.dropout(
            self.non_linearity_relu(self.batch_normalization_2(self.conv_layer_2(X)))
        )
        X = self.dropout(
            self.non_linearity_relu(self.batch_normalization_3(self.conv_layer_3(X)))
        )
        X = self.dropout(
            self.non_linearity_relu(self.batch_normalization_4(self.conv_layer_4(X)))
        )
        X = self.dropout(
            self.non_linearity_relu(self.batch_normalization_5(self.conv_layer_5(X)))
        )
...
class CPC_encoder_mlp(Module):
    ...
    def __init__(
        ...
        if normalization_type == "batchnorm":
            ...
            sys.exit(
                f'Wrong value for argument "normalization_type": {normalization_type}'
            )

        self.linear_layer_1 = Linear(
            in_features=linear_1_input_dim, out_features=linear_1_output_dim
        )
        self.normalization_1 = normalization_layer(num_norm_features_1)

        self.linear_layer_2 = Linear(
            in_features=linear_2_input_dim, out_features=linear_2_output_dim
        )
        self.normalization_2 = normalization_layer(num_norm_features_2)

        self.linear_layer_3 = Linear(
            in_features=linear_3_input_dim, out_features=linear_3_output_dim
        )
    ...
    def forward(self, X):
        ...
        for i in range(X.size()[1]):
            X_frame = X[:, i, :]
            X_frame = self.dropout(
                self.non_linearity_elu(
                    self.normalization_1(self.linear_layer_1(X_frame))
                )
            )
            X_frame = self.dropout(
                self.non_linearity_elu(
                    self.normalization_2(self.linear_layer_2(X_frame))
                )
            )
            X_frame = self.dropout(
                self.non_linearity_elu(
                    self.normalization_3(self.linear_layer_3(X_frame))
                )
            )
...
class CPC_postnet(Module):
    ...
    def __init__(
        self, encoding_dim=512, ar_model_output_dim=256, future_predicted_timesteps=12
    ):
        ...
        if isinstance(future_predicted_timesteps, int):
            ...
            self.W = ModuleList(
                [
                    Linear(
                        in_features=ar_model_output_dim,
                        out_features=encoding_dim,
                        bias=False,
                    )
                    for i in np.arange(future_predicted_timesteps)
                ]
            )
            ...
            self.W = ModuleList(
                [
                    ...
                    for i in range(len(future_predicted_timesteps))
                ]
            )

        else:
            sys.exit(
                'Configuration setting "future_predicted_timesteps" must be either an integer or a list of integers!'
            )
