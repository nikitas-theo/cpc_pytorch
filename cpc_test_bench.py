...
conf_file_names = [
    filename for filename in filenames_in_dir if filename.endswith(".py")
]
...
for conf_file in conf_file_names:
    os.system(
        f'echo -n "Running CPC experiment {conf_file_index}/{len(conf_file_names)} "'
    )
    os.system(
        f'echo "using configuration file {conf_file} (see log file of {conf_file} for further details)"'
    )
