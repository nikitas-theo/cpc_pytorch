...
from torch.nn import LogSoftmax, Module
...
class CPC_loss_no_classes(Module):
    ...
    def __init__(self, future_predicted_timesteps=12):
        ...
        if isinstance(future_predicted_timesteps, int):
            ...
            self.future_predicted_timesteps = np.arange(
                1, future_predicted_timesteps + 1
            )
    ...
    def forward(self, Z_future_timesteps, predicted_future_Z):
        ...
        for k in self.future_predicted_timesteps:
            loss -= diag(
                log_smax(
                    matmul(
                        Z_future_timesteps[k - 1], predicted_future_Z[i].transpose(0, 1)
                    )
                )
            ).sum(dim=0)
            i += 1
    ...
    def forward(self, Z_future_timesteps, predicted_future_Z, batch_labels):
        ...
        for k in self.future_predicted_timesteps:
            loss -= diag(
                log_smax(
                    matmul(
                        Z_future_timesteps[k - 1, indices_with_unique_label],
                        predicted_future_Z[i, indices_with_unique_label].transpose(
                            0, 1
                        ),
                    )
                )
