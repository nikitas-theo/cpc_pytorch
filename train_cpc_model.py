...
# Import our models
CPC_encoder = getattr(
    __import__("cpc_model", fromlist=[conf.encoder_name]), conf.encoder_name
)
CPC_autoregressive_model = getattr(
    __import__("cpc_model", fromlist=[conf.autoregressive_model_name]),
    conf.autoregressive_model_name,
)
CPC_postnet = getattr(
    __import__("cpc_model", fromlist=[conf.postnet_name]), conf.postnet_name
)

# Import our dataset for our data loader
CPC_dataset = getattr(
    __import__("cpc_data_loader", fromlist=[conf.dataset_name]), conf.dataset_name
)
...
optimization_algorithm = getattr(
    __import__("torch.optim", fromlist=[conf.optimization_algorithm]),
...
if conf.use_lr_scheduler:
    scheduler = getattr(
        __import__("torch.optim.lr_scheduler", fromlist=[conf.lr_scheduler]),
        conf.lr_scheduler,
    )
...
if __name__ == "__main__":
    ...
    model_parameters = (
        list(Encoder.parameters()) + list(AR_model.parameters()) + list(W.parameters())
    )
    optimizer = optimization_algorithm(
        params=model_parameters, **conf.optimization_algorithm_params
    )
    ...
    if conf.train_model:
        ...
        validation_set = CPC_dataset(
            train_val_test="validation", **conf.params_validation_dataset
        )
    ...
    if isinstance(conf.future_predicted_timesteps, int):
        ...
        timesteps = np.arange(
            conf.num_frames_encoding - conf.future_predicted_timesteps
        )
        ...
        for epoch in range(1, conf.max_epochs + 1):
            ...
            for train_data in train_data_loader:
                ...
                for t in timesteps:
                    ...
                    if batch_labels != []:
                        loss = loss_function(
                            Z_future_timesteps, predicted_future_Z, batch_labels
                        )
                    else:
            ...
            if conf.use_lr_scheduler:
                ...
                if current_learning_rate != learning_rate:
                    ...
                    with open(conf.name_of_log_textfile, "a") as f:
                        f.write(
                            f"Updated learning rate after epoch {epoch} based on learning rate scheduler, now lr={learning_rate}\n"
                        )
    ...
    if conf.test_model:
        ...
        try:
            Encoder.load_state_dict(
                load(conf.encoder_best_model_name, map_location=device)
            )
            AR_model.load_state_dict(load(conf.ar_best_model_name, map_location=device))
            W.load_state_dict(load(conf.w_best_model_name, map_location=device))
