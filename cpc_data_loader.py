...
class CPC_raw_audio_dataset(Dataset):
    ...
    def __init__(
        ...
        np.random.seed(
            2 * random_seed
        )  # We use a different random seed for splitting trainval_files
    ...
    def __getitem__(self, index):
        ...
        part_index = np.random.randint(
            len(self.feats[index]) - self.audio_window_length + 1
        )

        return from_numpy(
            self.feats[index][part_index : (part_index + self.audio_window_length)]
        ), []
...
class CPC_logmel_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        part_index = np.random.randint(
            len(self.feats[index]) - self.num_logmel_frames + 1
        )

        return from_numpy(
            self.feats[index][part_index : (part_index + self.num_logmel_frames)]
        ), []
